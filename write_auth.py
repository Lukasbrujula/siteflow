content = open('src/api/routes/auth.js', 'w')
content.write("""require('dotenv').config();
...
router.post('/request-otp', async (req, res) => {
  try {
    ...
    const tenant = db.prepare('SELECT * FROM tenants WHERE email = ?').get(email);
    if (!tenant) return res.status(404).json({ error: 'No account found for this email' });
    ...
    db.prepare('INSERT INTO login_tokens (token, tenant_id, expires_at) VALUES (?, ?, ?)').run(token, tenant.id, expiresAt);

    await transporter.sendMail({
      from: process.env.SMTP_USER,
      to: email,
      subject: 'Your SiteFlow login code',
      text: 'Your login code is: ' + token + '. Valid for ' + OTP_EXPIRY_MINUTES + ' minutes.',
    });
...
router.post('/verify-otp', (req, res) => {
  try {
    ...
    const record = db.prepare('SELECT * FROM login_tokens WHERE token = ? AND tenant_id = ? AND used = 0 AND expires_at > ?').get(token, tenant.id, now);
    ...
    db.prepare('INSERT INTO sessions (id, tenant_id, expires_at) VALUES (?, ?, ?)').run(sessionId, tenant.id, expiresAt);
...
router.get('/me', (req, res) => {
  ...
  const tenant = db.prepare('SELECT id, email, role FROM tenants WHERE id = ?').get(session.tenant_id);
