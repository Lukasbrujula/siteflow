content = open('src/workflow/index.js', 'w')
...
async function processEmail(email) {
  ...
  db.prepare('UPDATE emails SET status = ? WHERE id = ?').run('processing', email.id);
  ...
  try {
    ...
    const sentiment = triage.sentiment ? JSON.stringify(triage.sentiment) : null;
    const urgency = triage.sentiment?.urgency ? String(triage.sentiment.urgency) : null;

    if (classification === 'SPAM' || classification === 'AD') {
      db.prepare('UPDATE emails SET status = ?, classification = ?, sentiment = ?, urgency = ? WHERE id = ?')
    ...
    db.prepare('UPDATE emails SET status = ?, classification = ?, sentiment = ?, urgency = ?, draft_reply = ? WHERE id = ?')
    ...
    db.prepare('UPDATE emails SET status = ? WHERE id = ?').run('error', email.id);
  }
}

async function runWorkflowCycle() {
  const pending = db.prepare('SELECT * FROM emails WHERE status = ? LIMIT 5').all('pending');
  if (pending.length > 0) {
